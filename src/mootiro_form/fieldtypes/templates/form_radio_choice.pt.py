...
def bind():
    ...
    _re_amp = _loads("cre\n_compile\np1\n(S'&(?!([A-Za-z]+|#[0-9]+);)'\np2\nI0\ntRp3\n.")
    _attrs_61169744 = _loads('(dp1\nVclass\np2\nVdeformSet-item\np3\ns.')
    _attrs_60251536 = _loads('(dp1\nVclass\np2\nVdeformSet\np3\ns.')
    _attrs_60252048 = _loads('(dp1\nVtype\np2\nVhidden\np3\nsVname\np4\nV__end__\np5\ns.')
    _attrs_61170064 = _loads('(dp1\nVfor\np2\nV${field.oid}-${repeat.choice.index}\np3\ns.')
    _init_stream = _loads('cchameleon.core.generation\ninitialize_stream\np1\n.')
    _attrs_60251664 = _loads('(dp1\nVname\np2\nV__start__\np3\nsVtype\np4\nVhidden\np5\nsVvalue\np6\nV${field.name}:rename\np7\ns.')
    _init_default = _loads('cchameleon.core.generation\ninitialize_default\np1\n.')
    _attrs_61169936 = _loads('(dp1\nVname\np2\nV${field.oid}\np3\nsVvalue\np4\nV${value}\np5\nsVtype\np6\nVradio\np7\nsVid\np8\nV${field.oid}-${repeat.choice.index}\np9\ns.')
    ...
    def render(econtext, rcontext=None):
        ...
        u'None'
        _domain = None
        attrs = _attrs_60251536
        _write(u'<ul class="deformSet"> \n    ')
        attrs = _attrs_60251664
        ...
        for choice in _tmp1:
            ...
            (value, title, ) = choice
            _write(u'')
            attrs = _attrs_61169744
            _write(u'<li class="deformSet-item">\n          ')
            attrs = _attrs_61169936
            ...
            if ((_tmp3 is not None) and (_tmp3 is not False)):
                ...
                _write(((' id="' + _tmp3) + '"'))
            default = _default
            'str(value) == cstruct if cstruct else value == field.schema.opt_default'
            _tmp3 = ((str(value) == econtext['cstruct']) if econtext['cstruct'] else (value == _lookup_attr(_lookup_attr(econtext['field'], 'schema'), 'opt_default')))
            default = None
            if (_tmp3 is _default):
                _tmp3 = None
            if ((_tmp3 is not None) and (_tmp3 is not False)):
